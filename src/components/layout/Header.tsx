  ...
  return (
    <header className="relative z-50">
      <nav className="bg-[#0A1628] py-4 px-6 md:px-16 flex justify-between items-center relative">
        ...
        {/* Mobile Menu */}
        {isOpen && (
          <ul className="absolute top-full left-0 z-50 w-full bg-[#0A1628] flex flex-col items-center gap-4 py-6 md:hidden text-[#D1D5DC] text-sm">
            <li><Link to="/" onClick={closeMenu}>Home</Link></li>
            <li><Link to="/privacy" onClick={closeMenu}>Privacy Policy</Link></li>
            <li><Link to="/terms" onClick={closeMenu}>Terms of Service</Link></li>
          </ul>
        )}
      </nav>
    </header>
